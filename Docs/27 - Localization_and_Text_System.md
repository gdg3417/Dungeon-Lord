...
GD65B0C5 did not approve this pipeline ownership; GD65B0C6 does not physically author the six entries, any production file, Japanese path/table/translation, locale-selection behavior, language toggle, fallback implementation, diagnostic implementation, new schema version, or runtime behavior. Japanese has not yet been authored. Rows 53–58 of the approval register remain the exact terminology authority. After GD65B0C5 and GD65B0C6, numeric workload limits and production pipeline test ownership remained separate gates; GD65B0C7 later approved them without adding implementation.

## 9.4 GD65B0C7 language-workload and test ownership

`MaximumStringCharacters = 32768` is the single configuration-owned cumulative character budget for authored spatial-catalog strings and supplied localization lookup keys. Before publication, the production pipeline must use the same five-field `validation_limits.json` authority to count serialized language IDs, localization keys, localized values, and table-contract schema/content-version strings. Lookup entries also consume `MaximumNestedRecords = 512`. No sixth localization-specific production workload limit, code fallback, test-default authority, or Bootstrap fallback is approved.

The future pipeline suite must test production language-table accounting, ordinal key output, missing/null/blank/duplicate language assets and IDs, exactly one English fallback, missing keys, appendable valid test language packs without loader API changes, source non-mutation, deterministic export/reparse/revalidation, and no partial language publication. Data owns language fixtures and expected canonical output; Engineering owns implementation; QA owns the blocking verdict and evidence checklist; the primary developer supplies Unity evidence under `docs/testing/evidence/gd65b/`. GD65B0C7 adds no table, translation, test, loader, or runtime behavior.
