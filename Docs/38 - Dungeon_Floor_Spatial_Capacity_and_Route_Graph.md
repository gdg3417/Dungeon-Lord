...
**Inactive implementation boundary:** GD64 aligned the inactive layout contract to rectangular bounds, occupied-tile-union capacity, physical corridors, and footprint-free direct doorways. `FloorSpatialLayout` currently serializes room instances, route nodes, and route edges; `FloorLayoutValidator` materializes room and physical-corridor tiles. It does not serialize or materialize placed Entrance Hall or Completion Terminal instances with anchors and orientations. Required fixed-structure accounting is therefore approved production policy, not current passing validator behavior, and production activation must close this gap. GD65B0C3 does not alter this implementation boundary or the existing capacity-reduction safety and editor-display requirements. No schema, validation reason, runtime/save authority, content record, or migration is changed by this documentation packet.
...
The future dedicated `ContentService.LoadProductionSpatialContent(...)` path takes an explicitly injected read-only language-table collection, parses every table, ordinally validates language IDs, rejects null/blank/duplicate IDs, requires exactly one English fallback, validates all packs, and publishes no partial result. `GameRoot` owns four logical serialized inputs, with the language-table input as a collection whose only initial member is `string_table_en.json`; future packs append through data assignment without changing code, signatures, spatial IDs, or localization keys. Editor fallback and runtime discovery remain prohibited. Export, every-player-build preflight, and defensive runtime loading are three required gates sharing the same pure validation rules and configured limits. The future editor/command-line exporter canonicalizes the detached catalog, applies ordinal localization/manifest ordering, reparses and revalidates deterministic JSON, and uses same-filesystem staging, complete backups, a durable flushed journal, installed-set validation, and deterministic rollback or completion to publish exactly the catalog, initial English table, and manifest; no one-operation multi-file atomicity is claimed, and unfinished recovery blocks later export/build from a mixed set; builds validate committed files and never regenerate them.

The later GD65B1 slice creates only the separately authored limits file, strict parsing/conversion code, and initial workload/scalability tests; it creates no generated record, scene reference, or catalog activation. Existing abstract placement selections, ordered two-room layout, and room-slot assignments remain runtime/save authority; save schema remains 6. GD65B0C7 subsequently approves exact workload-limit values in rows 66–70 and production pipeline test ownership in row 72 without implementing them. GD65B0 is complete and GD65B implementation is unblocked but absent.
...
**Inactive corridor-validation boundary:** `SpatialContentValidation` checks that authored corridor width is positive and that minimum and maximum lengths are positive and ordered. The inactive `FloorLayoutValidator` resolves whether the referenced corridor definition ID exists, but then validates the supplied physical-edge footprint independently as a nonempty, unique, straight, contiguous line. It does not compare that footprint with the referenced definition’s `Width`, `MinimumLength`, or `MaximumLength`; consequently, the approved one-tile width and one-through-four-tile range are production content authority rather than current passing layout-validator behavior. Production activation must enforce referenced-definition geometry. A later implementation packet must add focused automated tests covering below-minimum, above-maximum, and definition/footprint mismatch cases. GD65B0C3 changes no validation implementation or reason values.
...
**Inactive implementation boundary:** current code has no `DeadEnd` enum member; capped-degree/direction, required-route, and direct-doorway rules are not validated; effective corridor loot is not topology-derived; corridor content instances and occupancy are not serialized or validated; overlap and terminal-tile placement are not enforced; and topology-edit repair does not exist. `FloorSpatialLayout` also does not serialize fixed-structure instances or count their footprints, does not enforce referenced corridor width/length against supplied edge footprints, and does not validate active connections against authored points/socket geometry. The production catalog remains inactive. Future validation reasons must append rather than renumber existing reasons. GD65B0C4 is documentation authority only: it changes no C#, save schema 6, runtime/save authority, migration, production record, or behavior.

# 6. Floor route graph

The required `spatial.fixed.entrance_hall` gross definition footprint is width 3 by height 2 (6 structural tiles), and the required `spatial.fixed.completion_terminal` footprint is width 2 by height 2 (4 structural tiles). Rows 43 and 48 of the GD65B register are the primary exact footprint authority, while rows 44–47 and 49–52 own their approved connectable profiles. Required fixed structures consume floor space once per occupied tile. Definition values remain distinct from anchors, runtime placement, and migration coordinates, which remain unapproved. The inactive `FloorSpatialLayout` still serializes no fixed-structure instances, and current layout validation therefore does not materialize or count them. This documentation approval authors no records and activates no behavior.
...
Spec 29 retains offline mana authority: apply the configured offline percentage through its single-grant calculation from the last known mana-per-hour rate, with no offline time cap and mana storage capacity as the output clamp. Research may improve efficiency percentage, storage capacity, or Mana Farm production, but not eligible offline duration. Percentage, timestamp/clock safeguards, and rounding remain later gates. The overnight-construction statement in the GD63 record is only a non-authoritative Phase 4 balance hypothesis.
...
`FloorSpatialConfiguration[]` remains an uncapped-by-schema array and `FloorIndex` remains an integer. Workload values are never catalog/save fields or gameplay limits. Raising them normally requires configuration review, evidence, validation, and an updated Unity content/application release, not by itself schema/save migration or architecture reconstruction. The catalog remains inactive, save schema remains 6, and existing abstract placements, ordered two-room layout, and room-slot assignments remain runtime/save authority. GD65B0 approval is complete; GD65B implementation has begun through GD65B1 with the approved limits asset, strict parser/conversion boundary, and initial workload/scalability tests. GD65B remains incomplete: generated catalog/table/manifest, exporter/deterministic bytes, recovery, loader/composition assignment, pre-build integration, and complete evidence remain absent. Save schema remains 6, the catalog and current runtime/save authority remain unchanged, and GD66 remains blocked on complete GD65B records, stages, tests, and evidence.
