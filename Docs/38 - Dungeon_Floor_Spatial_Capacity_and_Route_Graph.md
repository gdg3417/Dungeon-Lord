...
For the approved MVP design, each standard monster placement, each standard trap placement, and each loot placement physically occupies one authored usable room tile. Ordinary placements cannot overlap. Both the independently authored category maximum and available physical tiles must permit a placement; neither condition grants the other. Later authored content may use multi-tile footprints, including large monsters, bosses, large traps, environmental features, and large loot structures, but that implementation is post-MVP extensibility unless a later packet changes sequencing. The current inactive schema does not serialize individual monster, trap, or loot footprints or placement coordinates, so this policy is design authority rather than a claim of implementation support.

For the approved initial room profiles, simultaneous occupancy at all three ordinary-content category maxima is an authoring constraint of no more than 40% of gross footprint tiles, leaving at least 60% outside ordinary content occupancy. This is not a runtime-hardcoded percentage or a second weighted capacity system. Reserved and blocked tiles, usable-area rules, connection geometry, and clear connection-to-connection traversal remain separate authorities and requirements; remaining gross tiles are not automatically traversable. Exact production values and approval evidence remain centralized in the [GD65B approval register](../docs/planning/gd65b-production-spatial-content-approval.md), which records Basic Room at 6 / 16 (37.5%), Rectangle Room at 6 / 15 (40%), and Large Chamber at 12 / 30 (40%).
