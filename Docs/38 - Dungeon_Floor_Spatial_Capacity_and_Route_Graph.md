...
The initial MVP needs no coordinate-specific unavailable-tile mask. Its approved rectangle is the legal envelope; configured global capacity, structural fit, overlap, route validity, reserved tiles, and other authored placement restrictions remain independent constraints. This policy does not approve blocked regions, reserved tiles, or occupancy coordinates. A later explicit design may add an unavailable-coordinate mechanism.

The exact Floor 1 `FinalFloorSpaceCapacity` remains unapproved. Its room, straight-corridor, Entrance Hall, and Completion Terminal footprint dependencies are now approved, making the resulting intended structural composition envelope ready for a separate owner assessment; no value is selected here. Capacity may later increase through approved Architecture research, floor upgrades, or mana-funded expansion within unchanged bounds. Initial and maximum capacity, increments, formulas, costs, unlocks, modifier ordering, runtime behavior, and whether later progression changes bounds remain later gates.
...
The required `spatial.fixed.entrance_hall` gross definition footprint is width 3 by height 2 (6 structural tiles), and the required `spatial.fixed.completion_terminal` footprint is width 2 by height 2 (4 structural tiles). Rows 43 and 48 of the GD65B register are the primary exact production-value authority. Required fixed structures consume floor space once per occupied tile. Definition dimensions remain distinct from allowed orientations, reserved tiles, connection data, anchors, and runtime placement, all of which remain unapproved; the Completion Terminal being square does not approve orientations. The inactive `FloorSpatialLayout` still serializes no fixed-structure instances, and current layout validation therefore does not materialize or count them. This documentation approval authors no records and activates no behavior.
...
Exact Floor 1 final capacity; buildable-tile unlock and later unavailable-coordinate rules; expansion formulas/modifier behavior; remaining corridor capacities; reserved room tiles; orientations; connection counts and points; costs/modifiers; refund percentage/rounding/clamping; invested-mana save representation; migration coordinates/orientations/fixtures/fallbacks; branch formula/tie-break; transfer fields/formula/save state; offline-mana efficiency percentage, timestamp/clock safeguards, and rounding (without changing Spec 29’s no-time-cap/storage-clamp policy); and device limits require their named roadmap gates. Structural identities, ownership, movement/removal, direct-doorway, and one-tile-one-space policies are resolved in the GD63 decision record; GD65B0C1 later approved the initial room profiles and scoped occupancy policies; GD65B0C2 approved Floor 1 bounds and capacity-accounting policy while deferring exact capacity; GD65B0C3 later approved the initial Straight Stone Corridor geometry and required fixed-structure footprints without activating spatial behavior. Until approved in content/config and evidence, remaining values must not be guessed in runtime code.
