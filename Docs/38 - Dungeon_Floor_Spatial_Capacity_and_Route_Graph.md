...
The exact Floor 1 `FinalFloorSpaceCapacity` remains unapproved. It depends at least on approved straight-corridor width and length bounds, Entrance Hall and Completion Terminal footprints, and the resulting intended structural composition envelope. Capacity may later increase through approved Architecture research, floor upgrades, or mana-funded expansion within unchanged bounds. Initial and maximum capacity, increments, formulas, costs, unlocks, modifier ordering, runtime behavior, and whether later progression changes bounds remain later gates.
