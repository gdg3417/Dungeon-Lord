...
PR #183 / GD65B3A is merged and passed owner-run Unity validation. GD65B3B in this PR adds one shared invocation boundary used by the approved `Tools/Dungeon Lord/Content/Export Production Spatial Content` menu and Unity command line, and commits the deterministic `content_manifest.json`, `dungeon_spatial_content.json`, and `string_table_en.json` derived set. Normalized authoring remains writable authority and `validation_limits.json` remains separately authored configuration. Production loading, `ContentService` publication, `GameRoot` assignment, pre-build recovery/validation, runtime catalog consumption, and spatial gameplay activation remain absent. Loading a future validated catalog will not itself activate gameplay, UI, saves, placement, capacity, route graphs, construction, or simulation. Save schema remains 6, current runtime/save authority is unchanged, and GD66 remains blocked. Production loading and composition assignment is next. Exact-head owner Unity validation for GD65B3B remains required.
...
The gate is build-blocking: missing tests/evidence, changed limits, fallback authority, nondeterministic or invalid generated bytes, unresolved localization, mismatched manifest/schema/content versions, mixed sets, ignored journals, lossy recovery, bypassable pre-build checks, partial publication, Bootstrap fallback, generated limits, or a scalability fixture requiring code/schema changes blocks merge. Workload, scalability, deterministic export, recoverable publication/recovery, invocation, and committed-output responsibilities now have implementation and focused tests. Production loading, every-entry-point pre-build validation, and complete evidence remain absent. The three generated production files are committed by GD65B3B, but no owner Unity validation is claimed for this PR: the previously reported 1,273-test result predates proof that its three new tests were discovered, so exact-head validation must be rerun. Save schema remains 6, the catalog remains inactive, runtime/save authority is unchanged, and GD66 remains blocked until all required GD65B stages, tests, and evidence exist.
