...
GD65B1 completed the separately authored `validation_limits.json`, strict pure parser/conversion boundary, and initial workload-limit and scalability tests. GD65B2B then implemented the normalized authoring package and strict editor-only projection, and PR #182 completed deterministic in-memory construction plus strict complete-set reparse of the catalog, English table, and manifest. GD65B3A now implements the editor-only recoverable publication and recovery core with a durable strict journal, canonical installation order, complete-set readback validation, and deterministic recovery. No generated production JSON is committed, and editor-menu and Unity command-line entry points remain absent. Production loading, `GameRoot` composition assignment, pre-build integration, and complete evidence remain incomplete. Loading a future validated catalog will not activate gameplay, UI, saves, placement, capacity, route graphs, construction, or simulation. Save schema remains 6; the catalog remains inactive; current runtime and save authority remain unchanged; and GD66 remains blocked.
