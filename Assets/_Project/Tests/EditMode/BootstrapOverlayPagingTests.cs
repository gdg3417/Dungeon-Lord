...
namespace DungeonBuilder.Tests.EditMode
{
    public class BootstrapOverlayPagingTests
    {
        ...
        public void PlayerFacingDefault_ShowsPlayableMvpScreenSectionsAndHidesDiagnostics()
        {
            ...
            Assert.That(text, Does.Contain("Run posture:"));
        ...
        public void RunDiagnosticsFocus_FromPlayerFacingMode_RestoresPriorPlayerFacingModeSafely()
        {
            ...
            Assert.That(restoredText, Does.Contain("== Latest Result =="));
            Assert.That(restoredText, Does.Contain("== Top Status =="));
        ...
        [Test]
        public void ReplacingMonsterInRoomOne_ReportsRoomTargetedFeedback()
        {
            SetSave(NarrowHallTwoRoomSave(selectedRoomIndex: 0, includeRoomTwoLoot: false));
            Assert.That(_overlay.SelectMvpPlacementCategory(MvpDungeonPlacementIds.MonsterCategoryId), Is.True);
            Assert.That(_overlay.SelectMvpPlacementOption(MvpDungeonPlacementIds.SkeletonOptionId), Is.True);
        ...
        [Test]
        public void ReplacingTrapInRoomOne_ReportsRoomTargetedFeedback()
        {
            SetSave(NarrowHallTwoRoomSave(selectedRoomIndex: 0, includeRoomTwoLoot: false));
            Assert.That(_overlay.SelectMvpPlacementCategory(MvpDungeonPlacementIds.TrapCategoryId), Is.True);
            Assert.That(_overlay.SelectMvpPlacementOption(MvpDungeonPlacementIds.SpikeTrapOptionId), Is.True);
        ...
        public void DefaultPlayerFacingMode_PlayerPlacementAndRunActionsRemainAvailable()
        {
            ...
            Assert.That(runText, Does.Contain("== Latest Result =="));
        ...
        public void FirstSessionMvpUxFlow_HardensSelectionPlacementRunDiagnosticsAndRawIdBoundaries()
        {
            ...
            Assert.That(defaultText, Does.Contain("== Action Controls =="));
            Assert.That(defaultText, Does.Contain("== Top Status =="));
            ...
            _overlay.PlaceSelectedMvpStructure();
            Assert.That(_overlay.MvpStructurePlacementFeedback, Is.EqualTo("Changed Room 1 Monster: Empty -> Skeleton."));

            Assert.That(_overlay.SelectMvpPlacementCategory(MvpDungeonPlacementIds.TrapCategoryId), Is.True);
            Assert.That(_overlay.GetSelectedMvpPlacementPreviewText(), Is.EqualTo("Role: adds danger, heat, and path pressure."));
            _overlay.PlaceSelectedMvpStructure();
            Assert.That(_overlay.MvpStructurePlacementFeedback, Is.EqualTo("Changed Room 1 Trap: Empty -> Spike Trap."));

            _overlay.RunOrObserveDungeon();
            string playerFacingText = RefreshText();

            Assert.That(_overlay.MvpRunResultFeedback, Is.Not.Empty);
            Assert.That(playerFacingText, Does.Contain("Changed Room 1 Trap: Empty -> Spike Trap."));
            Assert.That(playerFacingText, Does.Contain("== Latest Result =="));
            ...
            Assert.That(focusedText, Does.Not.Contain("Changed Room 1 Trap: Empty -> Spike Trap"));
        ...
        [TestCase(MvpDungeonPlacementIds.RoomCategoryId, MvpDungeonPlacementIds.BasicRoomOptionId, "Room", "Basic Room", "Basic Room")]
        [TestCase(MvpDungeonPlacementIds.MonsterCategoryId, MvpDungeonPlacementIds.SkeletonOptionId, "Monster", "Skeleton", "Empty")]
        [TestCase(MvpDungeonPlacementIds.TrapCategoryId, MvpDungeonPlacementIds.SpikeTrapOptionId, "Trap", "Spike Trap", "Empty")]
        [TestCase(MvpDungeonPlacementIds.LootNodeCategoryId, MvpDungeonPlacementIds.BasicLootNodeOptionId, "Loot node", "Basic Loot Node", "Empty")]
        public void MinimalMvpActionPlacement_UsesSelectedPlacementBannerAndLocalizedSummary(string categoryId, string optionId, string categoryName, string displayName, string priorName)
        {
            DungeonLayoutState layout = DungeonLayoutState.CreateEmpty(1, 1);
            SetSave(new SaveData
            {
        ...
        public void PlacementFeedback_UpdatesAfterEachSelectedStructurePlacement()
        {
            ...
            Assert.That(_root.GetSelectedSlotStructureId(), Is.Empty);
            Assert.That(_overlay.MvpStructurePlacementFeedback, Is.EqualTo("Changed Room 1 Monster: Empty -> Skeleton."));
            Assert.That(text, Does.Contain("Changed Room 1 Monster: Empty -> Skeleton."));
        ...
        public void HiddenDiagnostics_F3OnlyChangesHiddenPageStateAndDoesNotChangePlayerFacingText()
        {
            ...
            Assert.That(after, Does.Contain("== Top Status =="));
        ...
        public void VisibleAndCopiedPlayerFacingSmokeTextDoesNotExposeRawIdsOrLocalizationKeys()
        {
            ...
            Assert.That(visible, Does.Contain("== Action Controls =="));
            Assert.That(visible, Does.Contain("== Latest Result =="));
            Assert.That(copied, Does.Contain("MVP Loop Summary"));
            Assert.That(copied, Does.Contain("Guided MVP Action"));
            Assert.That(copied, Does.Contain("First Dungeon Contract"));
            Assert.That(copied, Does.Contain("Room slot layout:"));
            Assert.That(copied, Does.Contain("Adventurer pressure"));
            Assert.That(copied, Does.Contain("Effects: none yet"));
        ...
        private static ContentService BuildContent(bool includeDiagnosticsLocalization)
        {
            ...
            map[MvpPlayableScreenPresenter.ActionControlsKey] = "Action Controls";
            map[MvpPlayableScreenPresenter.LatestResultKey] = "Latest Result";
            map[MvpPlayableScreenPresenter.DetailsHintKey] = "Details: press F5 to cycle focused sections, F6 to copy full smoke evidence, or show diagnostics from the action panel.";
            map[MvpPlayableScreenPresenter.RoomTargetControlFormatKey] = "Room target: {0}; {1}";
            map[MvpPlayableScreenPresenter.PlacementControlFormatKey] = "Placement: {0} / {1}";
            map[MvpPlayableScreenPresenter.PlaceButtonControlKey] = "Action button: Place / modify selected placement";
            map[MvpPlayableScreenPresenter.RunPostureControlFormatKey] = "Run posture: {0}";
            ...
            map["ui.mvp_screen.prompt.place_or_modify"] = "Action button: Place / modify selected placement";
