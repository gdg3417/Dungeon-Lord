...
namespace DungeonBuilder.Tests.EditMode
{
    public class CleanMvpValidationResetTests
    {
        ...
        private static void AssertCleanMvpBaseline(SaveData save)
        {
            ...
            Assert.That(save.completedObjectives.ObjectiveIds, Is.Null.Or.Empty);
            Assert.That(save.researchPending, Is.Null);
            Assert.That(save.researchProgress, Is.Null);
        }
        ...
        private static SaveData BuildDirtySave()
        {
            ...
            return new SaveData
            {
                ...
                completedObjectives = new CompletedObjectiveState { ObjectiveIds = new[] { "objective.first_dungeon_contract" }, LastCompletedObjectiveId = "objective.first_dungeon_contract" },
    ...
    public class CleanMvpValidationResetOverlayTests
    {
        ...
        public void ResetFromDevPanel_ClearsCachedFeedbackResetsSelectionAndRefreshesPlayerFacingSummary()
        {
            ...
            Assert.That(refreshed, Does.Contain("== Action Controls =="));
            Assert.That(refreshed, Does.Contain("Current heat: 0 (Peace)."));
            Assert.That(refreshed, Does.Contain("Action button: Place / modify selected placement"));
            Assert.That(refreshed, Does.Contain("Action button: Run / observe dungeon"));
        ...
        private static ContentService BuildOverlayContent(bool enableDevPanel)
        {
            ...
            map["guided_mvp.status.run_or_observe"] = "A dungeon placement is ready; observe adventurer activity next.";
