    ...
    public sealed class MvpRouteResultPresenterTests
    {
        ...
        public void BuildCompactText_ConfiguredTwoRoomRouteStoppedInRoomOneStillRenders()
        {
            var strings = new Dictionary<string, string> {
                ...
                [MvpRouteResultPresenter.RoomNumberFormatKey] = "Room {0}", [Gameplay.RunSimulation.RunSimulationService.RouteWipedKey] = "Party wiped." };
            var summary = new MvpPlayerLoopSummary { ConfiguredRoomCount = 2, ReachedRoomCount = 1, HighestRoomReached = 0, FinalRouteOutcomeKey = Gameplay.RunSimulation.RunSimulationService.RouteWipedKey };
            string text = MvpRouteResultPresenter.BuildCompactText(summary, (key, fallback) => strings.TryGetValue(key, out string value) ? value : fallback);
