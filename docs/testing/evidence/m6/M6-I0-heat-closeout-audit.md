...
1. **Resolved follow-up: active-run cooling composition.** The retained two-stage active-run cooling composition is recorded as intentional in [`M6-FU0: Active-run cooling composition`](../../../planning/decisions/M6-FU0-active-run-cooling-composition.md). Future changes require a separately scoped behavior PR.
...
M6 has the expected closeout shape: active heat delta calculation, active heat application, current-tier diagnostics, localized/read-only diagnostics, and Bootstrap diagnostics paging are implemented and covered by merged tests. No audit finding requires a documentation-only closeout to become NO-GO. The retained two-stage active-run cooling composition is now explicitly accepted in [`M6-FU0: Active-run cooling composition`](../../../planning/decisions/M6-FU0-active-run-cooling-composition.md). Any future behavior change requires a separately scoped PR. This follow-up does not authorize M7 to alter M6 heat behavior.
