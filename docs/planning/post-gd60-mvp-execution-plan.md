...
**Current status (2026-09-15):** Phase 3 is closed through merged PR #200. Phase 4A structural economy (#201), returned-content redeployment (#202), acquisition/StartingMana (#203), and direct content unassignment (#204) are merged and qualified. Current main baseline is PR #204 at `aac3af2a5836d22d62f9a4be5c2c27e9cf26c3ea`. This packet establishes canonical passive online mana earning; its implementation and qualification status are tracked in [the passive-online-mana evidence](../testing/evidence/phase4-passive-online-mana/static-review-evidence.md). Schema remains 9. Offline mana and run-event mana remain later work. Prior qualification evidence applies to its recorded commits, not this change. Earlier status blockers below are historical and superseded.
...
- **Economy/offline:** structural spending/refunds, owned-content redeployment, paid acquisition/StartingMana, and direct unassignment are merged/qualified through PRs #201–#204. This packet adds the canonical passive online mana/hour resolver and fractional active-tick award. Offline mana awaits its separate lifecycle/grant packet but must consume this resolver rather than duplicate the formula. Floor expansion remains deferred.
...
**Current passive online mana packet:** `passive_online_mana.json` owns the temporary MVP Core Level 1 input, 2 mana/Core Level/minute, 1 mana/active floor/minute, Peace 100% / Notice 95% / Concern 85%, and disabled soft-cap activation. The resolver derives active floors from validated canonical spatial state and heat from the existing heat authority. Formula order is Base → Heat → Research → Event/Season → Clamp/SoftCap → Rounding; Research and Event/Season are neutral, and disabled soft cap explicitly bypasses transformation. Under the current one-floor MVP state, rates are 180/171/153 mana/hour. The rounded hourly rate becomes a fractional award using the configured active tick, and the only wallet is clamped to structural economy capacity. The canonical mana/hour result is the future offline-rate input. Offline mana, run-event mana, durable Core Level progression, research mana effects, and active soft-cap tuning remain deferred. Combined Phase 4 exit criteria still require later packets.
