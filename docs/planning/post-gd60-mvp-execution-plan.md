...
**Current Phase 2B6A implementation status (2026-08-06):** PR #193 is merged at `d9d39871732f28f61c4958dc4a55fda922c66320`; detached candidate, transaction, and recovery are complete. Phase 2B6A adds the Windows durability implementation and activation-preflight boundary. It supports only Windows Editor and Windows Standalone with a local, nonredirected NTFS save directory, using write-through file creation plus explicit buffer flush and write-through same-directory native moves/replacements. Unsupported platforms, filesystem types, redirected/reparse paths, invalid paths, and native probe failures return stable fail-closed capability codes and no filesystem. Windows owner qualification is still required. Live schema remains **6**, schema 7 remains inactive, and `SaveService`, `GameRoot`, native creation, canonical runtime readers/writers, and legacy authority remain unchanged. Phase 2B6B is the final activation packet; GD66 is not complete.
...
| Current implementation baseline | Main through merged PR #193 at `d9d39871732f28f61c4958dc4a55fda922c66320` |
