...
**Current Phase 2B4B/2B5 implementation status (2026-08-03):** Phase 2B4A is merged. This PR adds an exact-source-bound preparation identity/candidate boundary, validated layout-contract/profile/geometry selection, bounded raw legacy route contracts, evidence-based frozen lower-model projection, R1/R2/implicit-container construction, marker-bound detached complete-save round-trip validation, and an explicitly invoked injected-filesystem transaction boundary. Recovery construction requires an immutable validation context for compatibility, production, validation-input, legacy-config, serializer, raw legacy classification, and workload authorities; no-journal legacy validation runs the real detached preparer so malformed effective route evidence is not trusted merely because its JSON envelope parses. Recovery restores verified O before returning pin/commit/durability codes whose registry contract requires O. Restoration persists and parses a canonical attempt-, descriptor-, O-, B-, journal-, and stage-bound intent before changing active authority; current-target self-valid C is selected before non-authoritative terminal audit evidence. Optional receipts use a bounded canonical three-field contract, durable directory flush and readback, and deterministic quarantine. Dedicated EditMode tests invoke the real preparer and real transaction `Execute`/`Recover` APIs with production compatibility/content assets and a normalized, platform-aware, operation-indexed injected filesystem; they cover empty schemas 1–6, unwrapped schema 1, active/persisted evidence, no-journal semantic rejection, and malformed-journal quarantine. The generic managed runtime filesystem uses bounded streaming enumeration and intentionally fails closed at directory durability; an engine-level test requires it to stop at `Replaced` with verified C rather than report `DurableVerified` or `Finalized`. Phase 2B6 must supply or select a supported platform implementation with a real directory-sync primitive before activation. The engine is inactive; live schema remains **6**, selected schema 7 remains inactive, and `SaveService`, `GameRoot`, ordinary no-file creation, runtime route readers/writers, and runtime room-content authority remain unchanged. Final live activation remains Phase 2B6; GD66 is not complete.
