...
**Current status (2026-09-26):** Phase 3 is closed through merged PR #200, Phase 4 is complete through merged PR #207, the Phase 5 design lock is merged in PR #208, and Phase 5A is complete in merged PR #209. The writable save schema is 10; it owns durable optional-branch topology, corridor trap/loot ownership, shared branch knowledge, and `ac_300`-gated construction/removal. Phase 5B is the next gameplay packet. Route choice, optional traversal, branch encounter resolution, branch-specific outcomes, run-driven knowledge learning, and production Phase 5B tuning remain unimplemented. PR #210 is merged and `Docs/process/AI_Model_Selection_Policy.md` is the canonical AI model-selection policy. Run-event mana, durable Core Level progression, research mana effects beyond the existing authored branching effect, active soft-cap tuning, and authoritative clock-cheat enforcement remain deferred. Earlier status blockers below are historical and superseded.
...
After merged Phase 5A PR #209, remaining MVP capabilities include subsequent acquisition balance review; run-event mana after its required authorities exist; Phase 5B optional-branch route choice/traversal, branch encounter resolution, branch-specific outcomes, and run-driven knowledge learning; Floor 2 transition; graphical editor parity; broader room/environment/content choices; broader Architecture progression; onboarding/accessibility; mobile profiling; and external fun-test evidence. Phase 5A supplies branch persistence/construction only and intentionally leaves required-route run outcomes unchanged.
...
4. **Merged and qualified in PR #207:** canonical idle/offline mana uses Spec 29’s locked single-grant calculation with no offline time cap and mana storage capacity as the output clamp; external UAT validation passed. Research may improve efficiency percentage, storage capacity, or Mana Farm production, not eligible duration. The player-facing result explains elapsed time, effective rate, storage-cap clamp and awarded mana; the overnight-one-edit statement remains only a non-authoritative balance hypothesis.
...
| Idle offline mana | Implemented, qualified, and externally UAT-validated in merged PR #207 | Preserve final MVP integration and security-monitoring follow-up without reopening the no-duration-cap policy | 4, 9 | Required |
