...
**Current Phase 2B6B closeout status (2026-08-21):** PR #195 implements the live schema-7 boundary using the production-owned save workload profile: raw-before-legacy recovery/migration for schemas 1–6, native canonical creation, lossless canonical lifecycle/player writes, production 2/2/2 additive capacity, runtime rebinding, frozen legacy spatial evidence, Narrow Hall repair-and-retry, and quiescent Dev Delete Save. Canonical spatial state is the sole writable spatial authority; `dungeonLayout` remains independent writable economic state. Required automated, Editor, Windows Player, and Windows x86_64 Development-build validation passed at `c4ba1f68985c18c2a6a62bcfd84c217e0cf07b06`. Production activation remains fail-closed outside qualified Windows Editor/Standalone local, nonredirected NTFS paths. GD66 is merged and complete at `4b8bb95ba3a5e202861e6aa4b3ec0e0cb427b34c`. Phase 3A in PR #196 currently implements the detached deterministic Basic Room R1→R2 domain preview/candidate path, fixed-structure capacity accounting, consequence disclosure, and stale-preview rejection. Live Bootstrap player controls and broader Rectangle/Large Chamber/corridor construction remain incomplete and must not be treated as implemented. Phase 3B replacement, renovation, movement, and removal consequence handling remains deferred. **Phase 3A identity correction (2026-08-21):** native player rooms use the centralized structural-only identity form `{floorId}.room.player.{ordinal:D4}`. The allocator derives the ordinal only from persisted room identities on the target floor, then verifies the candidate against the schema-7 save-wide instance-ID namespace (floors, rooms, nodes, edges, fixed structures, and assignments), so room-content assignments and `FloorRoomContentState.NextSequence` cannot affect it; canonical serialization preserves the allocated ID through reopen. Compatibility and migrated identities are unchanged. Structural deletion remains Phase 3B, and permanent reuse/non-reuse after deletion is a Phase 3B save-policy decision unless an approved specification resolves it earlier. No schema change is required for the Phase 3A operation set. Generalized placement and live player integration remain incomplete. **Direct-doorway native-placement blocker (2026-08-21):** Spec 38 requires the Completion Terminal after the final required room and forbids silent relocation, while the approved production catalog authors fixed-structure shapes/sockets but no native terminal-placement policy or anchors. The active compatibility evidence supplies terminal anchors only for the Basic Room R1/R2 straight-line variants. Consequently Rectangle Room, Large Chamber, and arbitrary Basic Room insertion cannot deterministically preserve the required linear route without either inventing terminal relocation geometry or creating an unapproved required branch. This blocker group remains unimplemented pending an approved native terminal-placement/route-insertion rule; the compatibility Basic R1→R2 regression remains the only authorized terminal move.**
...
| Field | Decision |
|---|---|
| Status | **GD66 / PR #195 merged and complete; Phase 3A remains in progress in PR #196** |
