...
5. **GD65B0 — Production spatial content authority and pipeline decision record (current dependency packet):** PR #168 established the [approval register](gd65b-production-spatial-content-approval.md), PR #169 approved its first identity group, PR #170 preserved branch-field extensibility, and GD65B0B approves metadata and Floor 1 references plus the current-scope Save boundary. Next address Floor 1 bounds, capacity, and room geometry/capacities unless evidence establishes a smaller dependency-correct group; then complete connections, localization, pipeline ownership, workload limits, and pipeline tests. Approval documentation authors no production records.
...
|---|---|---|
| GD65B remaining Floor 1 bounds/capacity, room/corridor/fixed geometry and capacities, concrete connections, localization, export/registration/loading ownership, validation/canonical serialization invocation, workload limits, and pipeline tests | GD65B0 approval before GD65B | Assigned Design, Data, Engineering, Localization, Save, Performance and QA owners; complete approval evidence in the GD65B record, never runtime constants or inferred prototype values |
