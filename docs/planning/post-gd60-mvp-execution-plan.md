...
| Field | Decision |
|---|---|
| Status | **PRs #201 and #202 merged/qualified; PR #203 acquisition/StartingMana merged and qualified** |
| Historical approval baseline | Main through merged PR #179 / GD65B1 at `917b763dc0e5315fdd5d835da4b5f5de43f9ba59` |
| Current implementation baseline | Main at merged PR #203 `eb7f918779267cb6b9a52e33374e814915ddd2e2` |
...
- **Economy/offline:** structural spending/refunds and free owned-content redeployment are merged/qualified through PRs #201 and #202. This packet adds approved tunable acquisition prices and StartingMana 40 through `Assets/_Project/Resources/content_acquisition_economy.json`; PR #203 is merged and qualified. Canonical online mana earning is a later dependency; offline mana awaits a meaningful canonical applicable production-rate authority. Floor expansion remains deferred.
...
After merged and qualified Phase 4A, remaining MVP capabilities include subsequent acquisition balance review; canonical online mana earning and then offline mana; one optional branch and route decision; Floor 2 transition; graphical editor parity; broader room/environment/content choices; Architecture progression; onboarding/accessibility; mobile profiling; and external fun-test evidence.

The current packet adds direct individual content unassignment after merged PR #203. A selected AssignmentId moves from its canonical room into returned custody under the existing authored ReturnToPlayerCustody policy, preserving category, option and sequence at zero mana with no structural changes. The approved current loot nodes are reusable owned sources, distinct from generated or resolved rewards. Bootstrap exposes individual canonical-order selection and free return/redeployment; qualification is tracked in the unassignment evidence linked above. Canonical online mana remains the next substantive Phase 4 dependency.
...
**Merged and qualified PR #202:** explicit redeployment reuses owned returned custody while preserving identity at zero mana cost. **Merged and qualified PR #203 acquisition packet:** configuration-owned prices are Skeleton 25, Goblin 20, Spike 20, Snare 15, Chilling Sigil 20, Basic Loot 15, Hidden Cache 10 and Glittering Hoard 25 mana; fresh native StartingMana is 40. Ordinary NEW acquisition atomically spends mana with fresh ownership and retains returned custody. NEW paid acquisition evaluates the existing RestrictedActionType.Purchase gate using current GameRoot.IsOnline and VerificationPending before canonical mutation; offline/pending attempts are blocked without state changes. Returned owned-content redeployment is not a new purchase and remains outside that gate. Repeated MONSTER acquisition is allowed while MonsterCapacity remains: every successful acquisition charges again and creates distinct AssignmentId/sequence; capacity rejection charges zero. Stale-session protection is separate from intentional current-session repeated acquisition. Trap/loot same-option live placement remains unchanged pending explicit approval. Existing saves receive no grant, structural investment/prices are unchanged, and schema remains 9. Exact authored data and transaction semantics are recorded in the [Phase 4A contract](phase-4a-structural-economy.md). Canonical online mana earning remains later; offline mana follows meaningful canonical online production authority. Active Architecture/floor/theme modifiers and expansion pricing remain deferred. Combined Phase 4 exit criteria still require later packets.
...
Final results and accepted limitations: [qualification evidence](../testing/evidence/phase4-content-acquisition/static-review-evidence.md).
