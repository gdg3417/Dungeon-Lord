...
| `MaximumCopiedSourceValueBytes` | cumulative raw bytes of recognized legacy primary values copied losslessly | strictly less than raw bytes because envelope, names, colons, and commas are not copied values | **262144 bytes** | intentionally nonbinding at the full raw ceiling | Its full-profile success boundary is unreachable. It remains an independent defense-in-depth guard for future profiles/composed contexts and produces a stable workload failure if configured lower; approval must use measured representative `copiedBytes` before deciding whether to lower it. Do not claim an exact 262144-byte full-pipeline success test. |
