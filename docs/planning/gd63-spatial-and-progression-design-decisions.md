...
The Phase 3B2A authored removal-policy field is carried by each MVP placement-effect definition. Current monster and trap definitions explicitly return to custody. Ownership survival remains unresolved, and therefore fail-closed, for `placement.option.loot_node.basic`, `placement.option.loot_node.hidden_cache`, and `placement.option.loot_node.glittering_hoard`; no loot disposition is inferred from their names or effects.
