...
**Approved Phase 4 clarification after merged PR #203:** the current Basic Loot Node, Hidden Cache and Glittering Hoard are reusable owned content, distinct from their generated rewards. Their authored removal records now authorize ReturnToPlayerCustody alongside monsters/traps. Direct unassignment preserves AssignmentId, category, option and sequence and costs zero mana; existing returned-content redeployment remains authoritative. The Phase 3B2 unresolved-loot statement below is historical and superseded for these three definitions. No generated-loot state or item inventory is added; schema stays 9 without migration.
...
The Phase 5 selection policy and deterministic tie-break are approved in the [Phase 5 branching and route-choice design lock](phase-5-branching-and-route-choice-design.md). The detailed authority locks the formula structure, confidence-boundary behavior, linear marginal mapping, and deterministic identity/hash/roll contract. Phase 5A branch structure and persistence are complete in PR #209. Production numeric tuning and run-condition semantics require owner approval in the [Phase 5B production tuning and run-condition contract](phase-5b-production-tuning-and-run-condition-contract.md); Phase 5B implementation, automated validation, manual gameplay validation, and merge qualification remain outstanding.
...
## Phase 3B1 stable-identity renovation rule (implemented 2026-08-22)

Movement targets a canonical player-built room by `RoomInstanceId`, derives its descendants from the unique required graph path from Entrance through Room nodes to Completion, and translates the selected room, downstream rooms, Completion Terminal, and internal physical corridors by one anchor delta. Replacement retains the target anchor and orientation, changes only to an allowed production room definition after content-capacity validation, and translates strict downstream structures only by the authored socket-position change needed to preserve the outgoing logical relationship. Changed boundary edges retain their IDs and endpoints and resolve to exactly one compatible Direct Doorway or approved Straight Stone Corridor; zero or multiple resolutions fail without search or repair. Room, node, edge, Completion Terminal, and content-assignment identities remain unchanged. Structural deletion is not part of this rule and remains blocked for Phase 3B2 policy.
