...
The exact `FinalFloorSpaceCapacity` remains unapproved and depends at least on rows 33–35, 43, and 48 plus the intended structural composition envelope. The inactive `FloorSpatialLayout` serializes rooms, route nodes, and route edges, and its validator currently materializes rooms and physical-corridor footprints only; placed Entrance Hall and Completion Terminal instances with anchors and orientations are not represented or counted. That fixed-structure accounting is an explicit inactive-contract implementation gap that must be closed before production activation. GD65B0C2 creates no records, changes no validation reason, and activates no layout, route graph, runtime reader, save authority, or migration.
...
This partial signoff does not open the GD65B gate. The dependency-correct next packet is **GD65B0C3: approve straight-corridor geometry and required fixed-structure footprints**, scoped to rows 33–35, 43, and 48 without approving orientations, reserved tiles, connections, sockets, capacities, or anchors. After that packet, exact Floor 1 capacity must be reassessed rather than inherited from an earlier planning candidate.

| Authority | Required decisions | Approver/evidence | Status |
|---|---|---|---|
| Design | Identities, geometry/capacity, structures, sockets/connections, prototype conflict | `gdg3417, primary developer and project owner`; scoped owner record above and this documentation PR | `PARTIAL — identities, Floor 1 references/branch allowance/bounds, room profiles, and capacity-accounting policy are approved; final capacity, remaining geometry, sockets, connections, localization, and other required decisions remain UNAPPROVED` |
| Data / Content Pipeline | Schema/content versions, IDs, export, manifest/registry, serialization | `gdg3417, primary developer and project owner`; scoped owner record above and this documentation PR | `PARTIAL — schema metadata, identities, Floor 1 references/bounds, room profiles, and capacity-accounting policy are approved; final capacity, export, manifest/registry, loading, canonical serialization ownership, and other required decisions remain UNAPPROVED` |
