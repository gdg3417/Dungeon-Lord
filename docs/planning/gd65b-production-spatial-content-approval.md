...
| Starting baseline for this approval update | `1a45e1a1454fcd7455d34e633bd735fc924fce7e` (main through merged PR #173 / GD65B0C2) |
...
| 17 | Final floor-space capacity | `UNAPPROVED` | Approved policy: a global occupied-structural-tile budget distinct from legal bounds. Used capacity is the deterministic union of room, physical-corridor, Entrance Hall, and Completion Terminal footprint tiles, counted once per coordinate; direct doorways add zero, and nested room contents add no second charge. The inactive validator currently materializes only room and physical-corridor tiles, not placed fixed-structure instances. | — | Exact configured `FinalFloorSpaceCapacity`, supported by a separate owner assessment of the now-approved room, corridor, and required fixed-structure geometry and intended structural composition envelope | Placement, progression, editor, performance | Design/Data | §4.2 and §14 approve accounting policy only; exact value remains pending |
| 18 | Floor 1 `AllowedRoomDefinitionIds` | `APPROVED` | Owner-approved Floor 1 foreign-key collection only; ordinal-canonical order | `["spatial.room.basic", "spatial.room.large_chamber", "spatial.room.rectangle"]` | Room gross footprints are approved in rows 23–25, and monster, trap, and loot capacities are approved in rows 29–31. Reserved tiles, usable-area derivation, orientations, connection limits and points, costs, localization, loading, saves, runtime assignment, placement, and activation remain separate or unapproved | Floor validation, authoring, editor catalog, canonical export | Design/Data | §14 owner metadata and Floor 1 approval record; this documentation PR |
| 19 | Floor 1 `AllowedCorridorDefinitionIds` | `APPROVED` | Owner-approved Floor 1 foreign-key collection only; ordinal-canonical order | `["spatial.corridor.straight_stone"]` | Straight Stone Corridor width and initial MVP length limits are approved in rows 33–35, and corridor monster capacity is approved in row 36. Trap capacity, loot capacity and dead-end eligibility, orientations, compatible sockets, costs, localization, loading, saves, runtime assignment, placement, and activation remain unapproved | Floor validation, authoring, editor catalog, canonical export | Design/Data | §14 owner metadata and Floor 1 approval record; this documentation PR |
| 20 | Floor 1 `EntranceStructureDefinitionId` | `APPROVED` | Owner-approved Floor 1 foreign-key assignment only | `EntranceStructureDefinitionId = "spatial.fixed.entrance_hall"` | Entrance Hall gross footprint is approved in row 43. Reserved tiles, allowed orientations, connection points, maximum connection count, anchors or coordinates, localization, runtime placement, migration placement, loading, saves, and activation remain unapproved | Floor validation, route endpoint, authoring, canonical export | Design/Data | §14 owner metadata and Floor 1 approval record; this documentation PR |
| 21 | Floor 1 `CompletionStructureDefinitionId` | `APPROVED` | Owner-approved Floor 1 foreign-key assignment only | `CompletionStructureDefinitionId = "spatial.fixed.completion_terminal"` | Completion Terminal gross footprint is approved in row 48; its square shape does not approve orientations. Reserved tiles, allowed orientations, connection points, maximum connection count, anchors or coordinates, localization, runtime placement, migration placement, loading, saves, and activation remain unapproved | Floor validation, route endpoint, authoring, canonical export | Design/Data | §14 owner metadata and Floor 1 approval record; this documentation PR |
...
| 43 | Entrance footprint | `APPROVED` | Owner-approved exact gross footprint for `spatial.fixed.entrance_hall`; the serialized 3 by 2 definition is authoritative and rotation remains separate | `spatial.fixed.entrance_hall`:<br>`GrossFootprint.Width = 3`<br>`GrossFootprint.Height = 2` | Reserved tiles, allowed orientations, connection points, maximum connection count, anchors, runtime placement, migration, and localization remain excluded | Occupancy, future placement, editor | Design/Data | §14 GD65B0C3 owner corridor-geometry and fixed-footprint approval; this documentation PR |
...
| 48 | Completion footprint | `APPROVED` | Owner-approved exact gross footprint for `spatial.fixed.completion_terminal`; its square shape does not approve an orientation set | `spatial.fixed.completion_terminal`:<br>`GrossFootprint.Width = 2`<br>`GrossFootprint.Height = 2` | Reserved tiles, allowed orientations, connection points, maximum connection count, anchors, runtime placement, migration, and localization remain excluded | Occupancy, future placement, editor | Design/Data | §14 GD65B0C3 owner corridor-geometry and fixed-footprint approval; this documentation PR |
...
The exact `FinalFloorSpaceCapacity` remains unapproved. With rows 33–35, 43, and 48 now approved, its structural-geometry dependencies are ready for the separate GD65B0C4 owner assessment of the intended composition envelope. The inactive `FloorSpatialLayout` serializes rooms, route nodes, and route edges, and its validator currently materializes rooms and physical-corridor footprints only; placed Entrance Hall and Completion Terminal instances with anchors and orientations are not represented or counted. That fixed-structure accounting is an explicit inactive-contract implementation gap that must be closed before production activation. That earlier packet created no records, changed no validation reason, and activated no layout, route graph, runtime reader, save authority, or migration.
...
Every occupied physical-corridor tile and every occupied required fixed-structure tile counts once against floor-space capacity. A direct doorway remains a separate connection kind with no corridor definition, no footprint, and zero floor-space use. Exact Floor 1 `FinalFloorSpaceCapacity` remains unapproved in row 17; these dimensions do not select, recommend, or imply it.

Orientations, reserved tiles, connection points and maximum connections, compatible sockets and socket compatibility, anchors or coordinates, runtime placement, and validator workload limits remain independently serialized or configured and unapproved. In particular, the Completion Terminal's square footprint does not approve any orientation. This documentation packet authors no production records, changes no save schema or authority, and activates no spatial catalog, layout, route graph, runtime reader, validation behavior, or migration. The inactive `FloorSpatialLayout` still has no fixed-structure instances, so current validation does not yet count the approved required fixed-structure footprints. Existing capacity-reduction safety and editor-display requirements remain unchanged.
...
This partial signoff does not open the gate, author records, activate placement, or claim current schema support. **GD65B remains blocked.** GD65B0C2 subsequently approves Floor 1 legal bounds and capacity-accounting policy while leaving exact capacity unapproved.
...
This partial signoff did not open the GD65B gate. GD65B0C3 subsequently approves rows 33–35, 43, and 48 without approving orientations, reserved tiles, connections, sockets, capacities, or anchors; exact Floor 1 capacity remains a separate reassessment rather than an inherited planning candidate.
...
This partial signoff does not open the GD65B gate. **GD65B remains blocked.** The dependency-ready next packet is **GD65B0C4: approve exact Floor 1 final floor-space capacity**; it must reassess the approved structural composition envelope rather than inherit or select a value here.

| Authority | Required decisions | Approver/evidence | Status |
|---|---|---|---|
| Design | Identities, geometry/capacity, structures, sockets/connections, prototype conflict | `gdg3417, primary developer and project owner`; scoped owner record above and this documentation PR | `PARTIAL — identities, Floor 1 references/branch allowance/bounds, room profiles, corridor geometry, fixed-structure footprints, and capacity-accounting policy are approved; final capacity, remaining capacities, orientations, sockets, connections, localization, and other required decisions remain UNAPPROVED` |
| Data / Content Pipeline | Schema/content versions, IDs, export, manifest/registry, serialization | `gdg3417, primary developer and project owner`; scoped owner record above and this documentation PR | `PARTIAL — schema metadata, identities, Floor 1 references/bounds, room profiles, corridor geometry, fixed-structure footprints, and capacity-accounting policy are approved; final capacity, export, manifest/registry, loading, canonical serialization ownership, and other required decisions remain UNAPPROVED` |
