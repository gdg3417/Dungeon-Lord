...
12. Create, flush, reread, and hash-verify the optional immutable audit receipt containing transaction ID, descriptor fingerprint, and the initially finalized C hash; advance the journal to `Finalized`, then apply cleanup/retention. If restoration occurs, verify O before `OriginalRestored`. The receipt never rewrites C and is never canonical authority.
...
On every load, discovery of a live non-finalized journal takes precedence and §21 recovery runs first. Otherwise a canonical save—migrated, native, empty, or later ordinarily modified—loads with `gd66.success.already_committed` when its supported schema, marker, creation kind, persisted layout-contract version, IDs/order, saved definition references, graph/contents, and current supported content rules validate. Historical candidate-hash equality and receipt presence/freshness are not checked. Missing, pruned, stale, or copy-omitted receipts do not invalidate the save; stale receipts may be quarantined/pruned as audit cleanup without a player failure. Ordinary atomic gameplay saves freely change active bytes while preserving marker/audit identifiers and layout-contract version.

Cloud copy, manual backup, reinstall restore, and save-file-only transfer therefore work from the complete active save alone. Historical profiles, geometry, manifests/catalog bytes, configs, and serializer implementations need not remain shipped after finalization; future compatibility uses explicit target-schema/content migrations.
...
| `gd66.diagnostic.finalized_backup_quarantined` | nonfatal diagnostic | No | Verified C | Allowed | No | Retained B is invalid after canonical C finalized; quarantine/remove B without affecting load. |
...
| `gd66.transaction.finalization_receipt_invalid` | recoverable failure | No | Verified C | Blocked | Yes | Before journal Finalized, a malformed/premature receipt conflicts with the exact live attempt; ignore/quarantine it and recover from journal evidence. |
...
| No live journal; complete finalized/native/later-modified canonical save; receipt present, missing, stale, or pruned | self-contained current target validation; receipt irrelevant | Verified C normal load; pins/receipt not required; save-file-only transfer valid | retain C; quarantine/prune stale audit | `gd66.success.already_committed` | No | No |
...
| Journal reaches `Finalized` after durable verification | journal/descriptor and initial C hash valid | Verified C; historical pins no longer required | retain C; archive/prune journal/receipt per policy | `gd66.success.migrated` | No | No |
| Live journal before `Finalized`; receipt malformed/premature | journal/pins and durable C/B evidence | ignore/quarantine receipt; continue exact live-journal recovery | quarantine receipt | `gd66.transaction.finalization_receipt_invalid` | Yes with exact pins | Maybe |
...
| Invalid retained B; canonical C finalized and self-valid | current self-contained target validation | Verified C normal load; B irrelevant | quarantine/remove B as audit cleanup | `gd66.diagnostic.finalized_backup_quarantined` | No | No |
| Invalid B before finalization; separate O verified | verified O independently matches descriptor | Unchanged Verified O; abandon/quarantine attempt | retain O; quarantine B/C/journal | `gd66.transaction.backup_failed` | Fresh attempt | No |
| Invalid B before finalization; no verified O; provisional C can complete | exact journal/pins/C evidence through Replaced or DurableVerified | gameplay blocked until C finalizes; otherwise no trusted payload | quarantine B; retain live evidence | `gd66.transaction.rollback_source_missing` | Continue only exact attempt | Maybe |
...
| Generic required unfinished validation/serializer input missing | exact descriptor input unavailable | restore Verified O; never substitute | quarantine C/journal; retain O/B | `gd66.transaction.pinned_input_missing` | When pin restored | Maybe |
| Generic required unfinished validation/serializer input hash mismatch | descriptor input exists but canonical hash/version differs | restore Verified O; never substitute | quarantine C/journal; retain O/B | `gd66.transaction.pinned_input_hash_mismatch` | When pin restored | Maybe |
| Unfinished migration profile missing | exact profile/geometry identity unavailable | restore Verified O; never substitute | quarantine C/journal; retain O/B | `gd66.transaction.pinned_profile_missing` | When pin restored | Maybe |
| Unfinished migration profile hash mismatch | exact profile exists but hash differs | restore Verified O; never substitute | quarantine C/journal; retain O/B | `gd66.transaction.pinned_profile_hash_mismatch` | When pin restored | Maybe |
| Unfinished manifest/catalog input missing | exact spatial input unavailable | restore Verified O; never substitute | quarantine C/journal; retain O/B | `gd66.transaction.pinned_spatial_input_missing` | When pin restored | Maybe |
| Unfinished manifest/catalog hash mismatch | exact spatial input exists but hash differs | restore Verified O; never substitute | quarantine C/journal; retain O/B | `gd66.transaction.pinned_spatial_input_hash_mismatch` | When pin restored | Maybe |
