...
### GD65B3B current status

PR #183 / GD65B3A is merged and passed owner-run Unity validation. GD65B3B adds the shared production spatial export invocation, the approved `Tools/Dungeon Lord/Content/Export Production Spatial Content` menu command, the Unity `-executeMethod` command-line entry point, and the first committed deterministic manifest/catalog/English generated set. The normalized package remains writable authority and `validation_limits.json` remains separate configuration authority. Production loading, `ContentService` publication, `GameRoot` assignment, pre-build recovery/validation, runtime catalog consumption, and spatial gameplay activation remain absent. Save schema remains 6; existing runtime/save authority is unchanged; GD66 remains blocked. The next planned packet is production loading and composition assignment.
